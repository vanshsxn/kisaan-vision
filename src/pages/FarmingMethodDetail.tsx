  ...
  return (
    <div className="min-h-screen bg-background pb-20 md:pb-0">
      ...
      <div className="relative h-[60vh] md:h-[70vh] overflow-hidden">
        <img src={method.heroImage} alt={method.title} className="absolute inset-0 w-full h-full object-cover" />
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 md:p-16">
          <div className="container mx-auto">
            ...
            <motion.span initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-6xl mb-4 block">{method.emoji}</motion.span>
            <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
              className="text-4xl md:text-6xl font-black text-foreground tracking-tight">{method.title}</motion.h1>
            <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
              className="text-primary font-semibold text-lg mt-2">{method.subtitle}</motion.p>
          </div>
        </div>
      </div>
        ...
        <div className="max-w-3xl mx-auto space-y-6 mb-20">
          {method.deepDescription.map((para, i) => (
            <motion.p key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
              transition={{ delay: i * 0.1 }} className="text-muted-foreground text-lg leading-relaxed">{para}</motion.p>
          ))}
        </div>
        ...
        <div className="mb-20">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {method.features.map((feat, i) => (
              <motion.div key={feat.title} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
                transition={{ delay: i * 0.1 }} whileHover={{ y: -4 }}
                className="rounded-2xl bg-card border border-border p-6 text-center shadow-sm hover:shadow-md transition-all">
