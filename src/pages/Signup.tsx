...
import AnimatedLogo from "@/components/AnimatedLogo"; // Import the component
...
const Signup = () => {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    setLoading(true); // Wait for verification state
    if (error) {
      setLoading(false);
        ...
        <div className="glass rounded-2xl p-8 glow-border">
          ...
          <form onSubmit={handleSignup} className="space-y-5">
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.98 }}
              className="w-full rounded-xl bg-primary py-3 text-sm font-bold text-primary-foreground flex items-center justify-center gap-2 hover:glow-green transition-all"
            >
              {loading ? (
