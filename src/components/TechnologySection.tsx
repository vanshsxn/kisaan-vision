...
const TechnologySection = () => {
  ...
  return (
    <section id="technology" className="relative py-24 overflow-hidden bg-[#030711]">
      {/* Background blur */}
      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        <div className="text-center mb-16">
          ...
          >
            Future of <span className="text-emerald-400">Farming</span>
          </motion.h2>
          <motion.p
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
          ...
          >
            Cutting-edge technologies <strong className="text-white">revolutionizing</strong> agriculture for a <strong className="text-white">sustainable tomorrow</strong>.
          </motion.p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {farmingMethods.map((tech, i) => (
            <motion.div
              ...
              className="group relative rounded-2xl overflow-hidden cursor-pointer border border-white/10 hover:border-emerald-500/30 transition-all duration-500 bg-white/5 backdrop-blur-sm"
            >
              {/* Background image */}
              <div className="relative h-44 overflow-hidden">
                ...
                {/* Icon badge */}
                <div className="absolute top-3 left-3 p-2 rounded-xl bg-white/10 backdrop-blur-md border border-white/20">
                  <tech.icon className="h-5 w-5 text-emerald-400" />
                </div>
              </div>

              {/* Content */}
              <div className="p-5">
                <h3 className="text-lg font-black text-white mb-0.5">{tech.title}</h3>
                <p className="text-xs font-bold text-emerald-400 mb-3">{tech.subtitle}</p>
                <p className="text-sm text-gray-400 leading-relaxed line-clamp-3">
                  {tech.description}
                </p>

                {/* Explore link */}
                <div className="mt-4 inline-flex items-center gap-1 text-sm font-bold text-emerald-400 group-hover:text-emerald-300 transition-colors">
                  Explore →
                </div>
              </div>

              {/* Bottom glow line */}
              <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-gradient-to-r from-transparent via-emerald-500/50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
            </motion.div>
          ))}
        </div>
      </div>
